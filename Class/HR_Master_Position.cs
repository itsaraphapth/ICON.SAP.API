...
** Modify Date      : 2019-05-17 09:54:53
 ** Version          : 1.0.0.10
*/
using System;
...
{
	[Serializable]
	[XmlRootAttribute("HR_Master_Position", Namespace = "http://www.iconframework.com",IsNullable = false)]
	public class HR_Master_Position
	{
		[XmlIgnore]
		[NonSerialized]
		public HR_Master_Position_Command ExecCommand = null;
		public HR_Master_Position()
		{
			ExecCommand = new HR_Master_Position_Command(this);
		}
		public HR_Master_Position(string ConnectionStr)
		{
			ExecCommand = new HR_Master_Position_Command(ConnectionStr, this);
		}
		public void InitCommand(string ConnectionStr)
		{
			ExecCommand = new HR_Master_Position_Command(ConnectionStr, this);
		}
		public HR_Master_Position(string ConnectionStr, Int32 Id_Value)
		{
			ExecCommand = new HR_Master_Position_Command(ConnectionStr, this);
	...
	public partial class HR_Master_Position_Command
	{
		string TableName = "HR_Master_Position";

		HR_Master_Position _HR_Master_Position = null;
		DBHelper _DBHelper = null;

		internal HR_Master_Position_Command(HR_Master_Position obj_HR_Master_Position)
		{
			this._HR_Master_Position = obj_HR_Master_Position;
			this._DBHelper = new DBHelper();
		}

		internal HR_Master_Position_Command(string ConnectionStr,HR_Master_Position obj_HR_Master_Position)
		{
			this._HR_Master_Position = obj_HR_Master_Position;
		...
		{
			foreach ( PropertyInfo ProInfo in _HR_Master_Position.GetType().GetProperties())
			{
				_HR_Master_Position.GetType().GetField("Edit" + ProInfo.Name).SetValue(_HR_Master_Position, false);
				if(dr.Table.Columns.Contains(ProInfo.Name))
				{
                    if (!dr.IsNull(ProInfo.Name))
                    {
					    ProInfo.SetValue(_HR_Master_Position,dr[ProInfo.Name],null);
					    //_HR_Master_Position.GetType().GetField("Edit" + ProInfo.Name).SetValue(_HR_Master_Position, true);
                    }
				}
			}
		}
		...
		public void Load(DBHelper _DBHelper ,Int32 Id_Value)
		{
            ...
            dt = _DBHelper.ExecuteDataTable("SELECT * FROM HR_Master_Position WHERE Id=@Id", Parameter);
			if (dt.Rows.Count > 0)
			{
				Load(dt.Rows[0]);
			}
        ...
        public void Load(DBHelper _DBHelper ,IDbTransaction Trans ,Int32 Id_Value)
		{
            ...
            dt = _DBHelper.ExecuteDataTable("SELECT * FROM HR_Master_Position WHERE Id=@Id", "HR_Master_Position", Parameter, CommandType.Text, Trans);
			if (dt.Rows.Count > 0)
			{
				Load(dt.Rows[0]);
			}
        ...
        {
            if (SelectQuery.SelectedTables.Length == 0) SelectQuery.SelectFromTable("HR_Master_Position");
            return _DBHelper.ExecuteDataTable(SelectQuery.BuildQuery(), CommandType.Text);
        }
        
        public List<HR_Master_Position> SelectAll()
        {
            return SelectAll(0, 0, null, null);
        }

        public List<HR_Master_Position> SelectAll(int start, int length, List<WhereClause> Where, List<OrderByClause> OrderBy)
        {
            ...
            Sqb.SelectFromTable("HR_Master_Position");
            if (Where != null)
            {
            ...
            List<HR_Master_Position> Res = new List<HR_Master_Position>();
            foreach(DataRow Dr in dt.Rows)
            {
                HR_Master_Position Item = new HR_Master_Position();
        ...
        private string GetInsertCommand(out DBParameterCollection Parameter)
        {
			...
			string ssql = "INSERT INTO HR_Master_Position (";
			string extColumn = "";
			string extParameter = "";
			Parameter = new DBParameterCollection();
			foreach (PropertyInfo ProInfo in _HR_Master_Position.GetType().GetProperties())
			{
				if ((bool)_HR_Master_Position.GetType().GetField("Edit" + ProInfo.Name).GetValue(_HR_Master_Position)  && ProInfo.Name != "Id")
				{
                    object Value = ProInfo.GetValue(_HR_Master_Position, null);
        ...
        public void Insert(DBHelper DBHelp, IDbTransaction DbTransaction)
		{
            ...
            if (SqlInsert == "") return;
            _HR_Master_Position.Id = int.Parse(DBHelp.ExecuteScalar(SqlInsert, Parameter, DbTransaction).ToString());
        ...
        public void Insert()
        {
            ...
            string SqlInsert = GetInsertCommand(out Parameter);
            if (SqlInsert == "") return;
            _HR_Master_Position.Id = int.Parse(_DBHelper.ExecuteScalar(SqlInsert, Parameter).ToString());
        ...
        {
            Parameter = new DBParameterCollection();
			string ssql = "UPDATE HR_Master_Position SET ";
			string extSql = "";
			foreach (PropertyInfo ProInfo in _HR_Master_Position.GetType().GetProperties())
			{
				if ((bool)_HR_Master_Position.GetType().GetField("Edit" + ProInfo.Name).GetValue(_HR_Master_Position)  && ProInfo.Name != "Id")
				...
				if (ProInfo.Name == "Id")
				{
					Parameter.Add(new DBParameter("@" + ProInfo.Name, ProInfo.GetValue(_HR_Master_Position, null)));
				}
			}
			ssql += extSql;
			ssql += " WHERE Id=@Id";
        ...
        {
            Parameter = new DBParameterCollection();
            string ssql = "DELETE HR_Master_Position WHERE Id=@Id";
            Parameter.Add(new DBParameter("@Id", _HR_Master_Position.Id));
